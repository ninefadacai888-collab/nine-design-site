...
class AuthService:
    ...
    async def get_or_create_user(self, platform_sub: str, email: str, name: Optional[str] = None) -> User:
        ...
        # Resolve auto-admin emails. Sources (merged):
        # 1) ADMIN_USER_EMAILS env (comma-separated list)
        # 2) Legacy ADMIN_USER_EMAIL env (single email)
        # 3) EXTRA_ADMIN_EMAILS hardcoded fallback for this project
        EXTRA_ADMIN_EMAILS = {
            "zxcvb8520258@gmail.com",
            "ninefadacai888@gmail.com",
        }
        admin_emails: set[str] = set(EXTRA_ADMIN_EMAILS)
        admin_emails_raw = os.environ.get("ADMIN_USER_EMAILS", "")
        if admin_emails_raw:
            admin_emails.update(e.strip().lower() for e in admin_emails_raw.split(",") if e.strip())
        legacy_single = getattr(settings, "admin_user_email", "") or ""
        if legacy_single:
            admin_emails.add(legacy_single.strip().lower())
        is_admin_email = bool(email) and email.strip().lower() in admin_emails
        ...
        if user:
            ...
            if is_admin_email and user.role != "admin":
                user.role = "admin"
                logger.info(f"Auto-promoted user {user.id} ({email}) to admin via ADMIN_USER_EMAILS")
        else:
            # Create new user
            ...
            self.db.add(user)
            if is_admin_email:
                logger.info(f"Auto-created admin user {platform_sub} ({email}) via ADMIN_USER_EMAILS")
